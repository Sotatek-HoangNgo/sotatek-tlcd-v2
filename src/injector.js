			...
			class Injector {
				...
				inject() {
					chrome.storage.local.get(
						...
						async (status) => {
							const toElement = document.getElementById("left-panel");
							const spaceWrapper = document.querySelector(".wrapper-spaces");

							if (!toElement) return;

							if (spaceWrapper) {
								toElement.style.display = "flex";
								toElement.style.flexDirection = "column";
								toElement.style.height = "calc(100vh - 80px)";

								spaceWrapper.style.flex = "1 1 auto";
							}
							...
							if (!container) {
								...
								toElement.insertAdjacentElement("beforeend", container);
							}
							if (!collapsedContainer) {
								...
								toElement.insertAdjacentElement("beforeend", collapsedContainer);
							}
							if (
								...
								this.lateMinutesOfMonth = await new Promise((resolve, reject) => {
									chrome.storage.local.get("employee_month_data", (data) => {
										...
										const records =
											(_b =
												(_a = data.employee_month_data) === null ||
												_a === void 0
													? void 0
